...
void ImageLoader::write_to_mem_seq(bluesy::ptr_type ram_ptr, uint32_t start_addr){
...
void ImageLoader::write_to_mem_interleaved(bluesy::ptr_type ram_ptr, uint32_t start_addr){
    DummyRAM* ram = (DummyRAM*) ram_ptr;
...
/*
    ...
    single 32 Bit word.
*/
void ImageLoader::write_to_mem_gray_packed(bluesy::ptr_type ram_ptr, uint32_t start_addr){
    DummyRAM* ram = (DummyRAM*) ram_ptr;
    uint32_t words = m_image.size() / 4;
    uint32_t rem = m_image.size() % 4;
    ...
    for(unsigned int i = 0; i < words; i++){
        uint32_t word = 0;
        word |= m_image[i*4];
        word |= (m_image[i*4+1] << 8);
        word |= (m_image[i*4+2] << 16);
        word |= (m_image[i*4+3] << 24);
    ...
    if(rem > 0){
        ...
        for(unsigned int i = 0; i < rem; i++){
            last_word |= m_image[words*4+i] << (8*i);
        }
...
BSV_WRAP_INSTANCE_METHOD(
    ...
    write_to_mem_seq,
    void,
    (bluesy::ptr_type, ram_ptr), (uint32_t, start_addr)
)
